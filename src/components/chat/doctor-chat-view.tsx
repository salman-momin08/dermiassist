...
const EmptyChat = () => {
    return (
        <div className="flex flex-col h-full items-center justify-center p-8 text-center bg-background">
            ...
            <h3 className="text-base font-bold text-foreground font-headline">Select a Patient Consultation</h3>
            <p className="text-xs text-muted-foreground mt-1 max-w-xs leading-relaxed">
                Choose an active patient consultation channel from the left sidebar to communicate directly with your registered patient.
            </p>
        </div>
    );
};
...
const EmptyChannelList = () => {
    return (
        <div className="flex flex-col h-full items-center justify-center p-6 text-center bg-muted/10">
            ...
            <p className="text-xs font-bold text-foreground">No Active Patient Consultations</p>
            <p className="text-[11px] text-muted-foreground mt-1 max-w-xs">
                New patient conversations will appear here automatically when consultations are initiated.
            </p>
        </div>
    );
};
...
const CustomChannelHeader = () => {
    ...
    // Find other member (Patient)
    const members = Object.values(channel?.state?.members || {});
    const otherMember = members.find((m: any) => m.user?.id !== client.userID)?.user;
    const channelName = (channel?.data as any)?.name || otherMember?.name || 'Registered Patient';
    ...
    return (
        <div className="flex items-center justify-between px-5 py-3.5 bg-card border-b border-border/80 shadow-2xs">
            {/* Left: Patient Avatar & Information */}
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                        Patient Consultation Channel · Direct Telehealth Line
                    </p>
                </div>
            </div>
        ...
        <div className="container mx-auto px-4 py-5 md:py-8 max-w-6xl flex flex-col h-[calc(100dvh-140px)] min-h-[580px]">
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">Direct encrypted messaging with your registered clinical patients.</p>
                </div>
            </div>
