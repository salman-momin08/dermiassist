...
export default function AnalyzeClient() {
  ...
  const getNextQuestion = async (
    ...
    setIsLoading(true);
    try {
      const { nextQuestion, suggestedAnswers, isComplete, confidenceScore: modelConfidence } = await proformaChat({
        conditionName: conditionName,
        conversationHistory: historyString,
      });

      // Update tailored suggestions dynamically generated by the AI agent
      if (suggestedAnswers && Array.isArray(suggestedAnswers) && suggestedAnswers.length > 0) {
        setDynamicSuggestions(suggestedAnswers);
      } else {
        generateQuestionSuggestions({
          ...
          console.warn("Suggestions agent error:", err);
        });
      }

      // Only update confidence from model when the user has actually answered at least once
      if (questionCount > 0) {
      ...
      // Note: questionCount is incremented in handleUserResponse when the user
      // actually answers, NOT here when the AI sends a question.
    } catch (err: any) {
  ...
  const handleUserResponse = (responseText?: string) => {
    ...
    // Increment question count only when user actually answers — this drives the progress bar
  ...
  const startProforma = (conditionName: string) => {
    ...
    generateQuestionSuggestions({
      question: `Initial consultation for ${conditionName}`,
      conditionName,
    }).then((res) => {
      if (res?.suggestions && res.suggestions.length > 0) {
        setDynamicSuggestions(res.suggestions);
      }
    }).catch((err) => console.warn("Suggestions agent initial error:", err));
    ...
    <div className="container mx-auto px-4 py-6 md:py-8 max-w-4xl pt-6 sm:pt-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <Button
          ...
          size="sm"
          onClick={resetState}
          className="h-9 px-4 rounded-full bg-card/90 backdrop-blur-md border-border/80 hover:bg-accent hover:border-primary/40 transition-all shadow-xs text-xs font-semibold gap-2"
        >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 bg-card/90 backdrop-blur-md px-3 h-9 rounded-full border border-border/80 shadow-xs hover:border-primary/40 transition-all">
            ...
            <>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="flex items-center gap-2.5 bg-card/90 backdrop-blur-md px-3.5 h-9 rounded-full border border-border/80 shadow-xs transition-all hover:border-primary/40">
                      <Volume2 className={cn("h-3.5 w-3.5 text-primary", speechMode && "animate-pulse")} />
                      <Label htmlFor="speech-mode" className="text-xs font-medium cursor-pointer select-none hidden sm:inline text-muted-foreground">
              ...
              {questionCount >= 1 && (
                <Button
                  ...
                  disabled={isLoading}
                  className="h-9 px-4.5 rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-indigo-700 hover:from-blue-500 hover:to-indigo-600 text-white font-semibold text-xs shadow-md shadow-indigo-500/20 border border-white/10 active:scale-95 transition-all gap-2"
                >
      ...
      <Card className="shadow-2xl border-border/80 overflow-hidden backdrop-blur-xl bg-card/90 rounded-2xl relative">
        ...
        <CardHeader className="border-b border-border/60 bg-muted/20 pb-4 pt-5 px-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {step === 'proforma' && detectedCondition && (
              <div className="flex items-center gap-2 bg-primary/10 text-primary border border-primary/20 rounded-full px-3.5 py-1.5 text-xs font-semibold backdrop-blur-sm shadow-xs">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-muted-foreground flex items-center gap-1.5">
                  ...
                  <span>Consultation Calibration{questionCount > 0 ? ` · ${questionCount} answered` : ''}</span>
                </span>
          ...
          <>
                  ...
                  {preview ? (
                    <div className="relative group flex flex-col items-center gap-4">
                      <div className="relative rounded-2xl overflow-hidden border-2 border-border/80 shadow-2xl shadow-black/20 group-hover:scale-[1.02] transition-transform duration-300">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                          <span className="text-white text-xs font-semibold bg-black/60 px-3 py-1.5 rounded-full backdrop-blur-md border border-white/20">
                      ...
                      <Button
                        variant="destructive"
                        size="sm"
                        className="rounded-full shadow-xs gap-1.5 text-xs font-semibold px-4 h-8"
                        onClick={(e) => {
                          e.stopPropagation();
                          setFile(null);
                          setPreview(null);
                    ...
                    <label htmlFor="picture" className="cursor-pointer w-full flex flex-col items-center justify-center space-y-4 py-6">
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                        <Badge variant="secondary" className="text-[11px] font-medium rounded-full px-3 py-1 bg-muted/60 border border-border/60">
            ...
            <CardFooter className="border-t border-border/60 bg-muted/10 p-5 flex flex-wrap items-center justify-between gap-3">
              ...
              <Button
                ...
                className="h-12 px-8 rounded-full font-bold text-sm bg-gradient-to-r from-blue-600 via-indigo-600 to-indigo-700 hover:from-blue-500 hover:to-indigo-600 text-white shadow-lg shadow-indigo-600/25 transition-all active:scale-[0.98] gap-2"
              >
                {isLoading ? (
                  <>
        ...
        {step === 'proforma' && (
          <div className="flex flex-col h-[65vh] min-h-[520px]">
            {/* Specimen Banner */}
            {preview && (
              <div className="bg-muted/30 px-5 py-2.5 border-b border-border/60 flex items-center justify-between text-xs text-muted-foreground backdrop-blur-md">
                <div className="flex items-center gap-3">
                  <div className="relative rounded-lg overflow-hidden border border-border/80 shadow-xs">
                    <Image
                      src={preview}
                ...
                <Badge variant="outline" className="text-[11px] font-semibold border-primary/40 bg-primary/5 text-primary rounded-full px-2.5 py-0.5">
                  AI Active Triage
                </Badge>
              </div>
            )}
              ...
              <div className="space-y-4 max-w-3xl mx-auto">
                {chatHistory.map((msg, index) => {
                  ...
                  return (
                    ...
                    >
                      {isAi && (
                        <Avatar className="h-8 w-8 border border-primary/30 bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-xs shrink-0 mt-0.5 rounded-full">
                      ...
                      {isAi ? (
                        <div className="flex flex-col items-start max-w-[85%] sm:max-w-[75%]">
                          ...
                          {/* Metadata row strictly beneath the message card */}
                          <div className="flex items-center gap-3 mt-1.5 ml-1">
                            <span className="text-[10px] text-muted-foreground/60 font-medium tracking-wide">{msg.timestamp || 'Just now'}</span>
                            <button
                              type="button"
                              onClick={() => handlePlayMessageAudio(msg.text)}
                              disabled={isAudioLoading === msg.text}
                              className={cn(
                                "inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-[10.5px] font-medium text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all border border-border/60 hover:border-primary/30 cursor-pointer shadow-2xs",
                                playingAudio?.text === msg.text && "text-primary bg-primary/10 border-primary/30 font-semibold"
                              )}
                            >
                              {isAudioLoading === msg.text ? (
                                <Loader2 className="h-2.5 w-2.5 animate-spin" />
                              ) : playingAudio?.text === msg.text ? (
                                <VolumeX className="h-2.5 w-2.5 text-primary" />
                              ) : (
                                <Volume2 className="h-2.5 w-2.5" />
                              )}
                              <span>{playingAudio?.text === msg.text ? 'Pause' : 'Listen'}</span>
                            </button>
                          </div>
                        </div>
                      ) : (
                        <div className="flex flex-col items-end max-w-[80%] sm:max-w-[70%] ml-auto">
                          ...
                          <span className="text-[10px] text-muted-foreground/60 font-medium tracking-wide mt-1.5 mr-1 text-right">{msg.timestamp || 'Just now'}</span>
                        </div>
                      )}

                      {!isAi && (
                        <Avatar className="h-8 w-8 border border-indigo-400/30 bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-xs shrink-0 mt-0.5 rounded-full">
                          <AvatarFallback className="bg-transparent text-white font-bold">
                ...
                {isLoading && (
                  <div className="flex items-start gap-3">
                    <Avatar className="h-8 w-8 border border-primary/30 bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-xs shrink-0 rounded-full">
                      <AvatarFallback className="bg-transparent text-white font-bold">
                        <Bot className="h-3.5 w-3.5" />
                      </AvatarFallback>
                    </Avatar>
                    <div className="w-fit rounded-2xl rounded-tl-xs px-4 py-3 bg-card border border-border/80 flex items-center gap-2 text-xs text-muted-foreground shadow-xs">
            ...
            {/* Smart Dynamic Quick Answer Suggestions */}
              ...
              <div className="px-5 py-2.5 border-t border-border/60 bg-muted/20 flex gap-2 overflow-x-auto scrollbar-none items-center">
                ...
                {dynamicSuggestions.map((chip, i) => (
                  <button
                    ...
                    className="h-8 px-4 rounded-full text-xs font-medium bg-card hover:bg-primary/10 hover:border-primary/50 hover:text-primary border border-border/80 transition-all shadow-xs hover:shadow-sm active:scale-95 text-foreground cursor-pointer flex items-center gap-2 shrink-0 backdrop-blur-md"
                  >
            ...
            <div className="p-4 border-t border-border/60 bg-card/90 backdrop-blur-md">
              <div className="relative flex items-center max-w-3xl mx-auto">
                <Input
                  ...
                  disabled={isLoading}
                  className={cn(
                    "pr-28 py-6 rounded-full bg-muted/30 focus-visible:ring-primary text-sm shadow-inner transition-all px-5",
                ...
                <div className="absolute right-2 flex items-center gap-1.5">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          disabled={isLoading}
                          className={cn(
                            "h-9 w-9 rounded-full transition-all border border-transparent hover:border-border/60 hover:bg-muted/80",
                  ...
                  <Button
                    ...
                    className="h-9 w-9 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white shadow-md shadow-indigo-600/20 hover:scale-105 active:scale-95 disabled:opacity-40 disabled:hover:scale-100 transition-all flex items-center justify-center"
                  >
            ...
            {step === 'error' && error && (
              <div className="space-y-4 max-w-md flex flex-col items-center text-destructive">
                ...
                <Button
                  ...
                  size="default"
                  onClick={resetState}
                  className="h-10 px-6 rounded-full font-semibold text-xs bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white shadow-md shadow-indigo-600/20 gap-2 mt-3 transition-all active:scale-95"
                >
